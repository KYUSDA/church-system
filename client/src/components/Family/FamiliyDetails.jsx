...
const SingleFamily = () => {
  ...
  const family = useSelector((state) => state?.families?.families)?.find((fam) => fam?._id === familyId);
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12">
      ...
      <section className="container mx-auto grid md:grid-cols-2 gap-6 px-4 items-center">
        ...
        <div className="flex justify-center">
          <img src={urlFor(family?.imgUrl)} alt="Family" className="w-full max-w-md rounded-lg shadow-xl" />
        </div>
      </section>
    ...
    <div className="w-3/4 max-w-7xl">
        ...
        <img src={urlFor(family?.locationUrl)} alt="Family Location" className="w-full rounded-lg shadow-md" />
        </div>
    </div>
    </section>
