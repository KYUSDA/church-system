...
const ADepartment: React.FC = () => {
  ...
  const handleClick = async(e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
        ...
        try {
          const response = await fetch(`${BASE_URL}/department/update-department/${department._id}`, {
            method: "PATCH",
            headers: {
  ...
  return (
    <div className="flex flex-col items-center p-6">
      ...
      <div className="w-full max-w-3xl bg-white shadow-lg rounded-xl p-6">
        ...
        <img
            src={department?.imgUrl ? urlFor(department.imgUrl).url() : "/placeholder.jpg"}
            alt={department?.title || "Department Image"}
            className="w-32 h-32 rounded-full object-cover border shadow"
          />
          <p className="text-gray-500 mt-2">{department?.description || "No description"}</p>
        </div>
